...
## 2026-05-18 - SSRF Vulnerability in Workflow Engine HTTP Request Node
**Vulnerability:** The `action.http_request` execution branch in `src/core/workflow_engine.py` directly fetched dynamically provided URLs via `urllib.request.urlopen` without any validation or filtering. This permitted a Server-Side Request Forgery (SSRF) attack where a malicious workflow configuration could force the backend to send requests to local network addresses or cloud metadata endpoints (e.g., `169.254.169.254`).
**Learning:** Functions that accept URLs from user or external configurations and fetch them server-side must always validate the destination before executing the request. SSRF vulnerabilities can expose internal infrastructure, allow unauthenticated local API access, or leak sensitive cloud credentials.
**Prevention:** Always parse untrusted URLs and enforce a whitelist of allowed schemes (`http`, `https`). Validate the extracted hostname against a strict blocklist of local/internal IP ranges (`localhost`, `127.0.0.1`, `10.*`, `192.168.*`, `172.16.0.0/12`) to prevent the server from interacting with internal endpoints.
