...
## 2024-05-30 - [CRITICAL] Server-Side Request Forgery (SSRF) in Workflow Engine
**Vulnerability:** The `action.http_request` node in `src/core/workflow_engine.py` blindly executed HTTP requests to user-provided URLs using `urllib.request.urlopen` without validating the destination. This could allow an attacker to probe internal networks, access metadata services (e.g., AWS IMDS), or bypass firewalls.
**Learning:** Never trust user-provided URLs for server-side HTTP requests. Even if the application logic expects an external URL, the underlying HTTP client will happily resolve and connect to internal IPs (like `127.0.0.1` or `169.254.169.254`) if instructed to do so. Furthermore, DNS rebinding and malicious redirects can bypass simple string-based checks.
**Prevention:** To prevent SSRF, always enforce explicit validation before making requests. Ensure the URL scheme is restricted (e.g., `http` and `https`), resolve the hostname to its underlying IPs using `socket.getaddrinfo`, and verify that none of the resolved IPs fall into private, loopback, link-local, multicast, or unspecified IP ranges (using the `ipaddress` module). Additionally, implement a custom `HTTPRedirectHandler` to enforce these exact same checks on any subsequent redirect URLs.
