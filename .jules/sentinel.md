...
## 2026-04-14 - Fix Server-Side Request Forgery (SSRF) in Workflow Engine
**Vulnerability:** The workflow engine executed arbitrary HTTP requests (`action.http_request`) based on user-provided configurations without validating the target URL. This could allow an attacker to bypass firewalls and make requests to internal services (e.g., `localhost`, `169.254.169.254`, `0.0.0.0`) by providing internal IPs or domains that resolve to them.
**Learning:** Using basic string matching or `urllib.parse` alone is insufficient to prevent SSRF because attackers can use DNS rebinding or obfuscated IPs (like `0.0.0.0` or octal/hex representations). Furthermore, redirects must also be validated to prevent attackers from bypassing initial checks by returning an HTTP 30x redirect to an internal IP.
**Prevention:** To prevent SSRF, always explicitly validate the URL scheme (`http`/`https`), resolve the hostname to an IP address using `socket.getaddrinfo()` (to support IPv6 and reliably catch `0.0.0.0`), and verify the resolved IP using `ipaddress` to block private, loopback, link-local, multicast, and unspecified addresses. Use a custom `urllib.request.HTTPRedirectHandler` to intercept and validate all redirects through the same check.
