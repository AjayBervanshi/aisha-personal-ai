...
## 2025-05-24 - Fix Server-Side Request Forgery (SSRF) in Workflow Engine
**Vulnerability:** The workflow engine (`src/core/workflow_engine.py`) executed HTTP requests for `action.http_request` nodes without validating the target URL. This allowed an attacker to request internal network addresses (e.g., `http://localhost`, `127.0.0.1`, AWS metadata `169.254.169.254`) and other non-HTTP schemes like `file://`.
**Learning:** Blindly executing web requests using user-provided URLs in backend infrastructure enables Server-Side Request Forgery. Attackers can leverage the application's privileged network position to bypass firewalls and access internal services or read local files. Relying simply on string blocklists is insufficient, as attackers can bypass them using different representations of IPs (e.g., DNS resolution or integer IPs).
**Prevention:** Always validate target URLs before sending requests. Ensure the scheme is strictly `http` or `https`. Resolve the URL's hostname to an IP address and block private, loopback, multicast, and link-local IP addresses using Python's `ipaddress` module. Finally, use a custom HTTP redirect handler (`SafeRedirectHandler`) to validate URLs encountered during redirects, as an attacker could provide an external URL that redirects to an internal IP.
