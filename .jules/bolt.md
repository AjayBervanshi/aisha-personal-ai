...
## 2024-05-19 - [Supabase Count Limit Overhead in Monitoring Engine]
**Learning:** Found exact count query URLs (e.g., `?select=id` with header `Prefer: count=exact`) in `src/core/monitoring_engine.py` missing the limit clause. This defaults to fetching all O(N) matched rows over the network into memory before computing the count.
**Action:** Always append `&limit=1` to query URLs (e.g., `?select=id&limit=1`) when requesting exact counts via HTTP PostgREST API to only return the count header without a heavy payload.
