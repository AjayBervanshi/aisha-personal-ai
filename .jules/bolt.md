...
## 2024-05-18 - [PostgREST count=exact HTTP Query Overhead]
**Learning:** Raw HTTP requests to Supabase PostgREST using `Prefer: count=exact` header without specifying a limit return all matching rows in memory before evaluating the exact count, causing O(N) memory and network overhead.
**Action:** Always append `&limit=1` (or limit=0 via postgrest-py `.limit(0)`) to the URL parameters (e.g. `?select=id&limit=1`) when doing count queries where only the count headers matter to prevent downloading massive result sets.
