...
def _check_stuck_queue() -> list:
    ...
    try:
        r = requests.get(
            ...
            f"?status=eq.queued&created_at=lt.{cutoff}&select=id&limit=1",  # Optimization: Added &limit=1 to count="exact" query to prevent loading all rows into memory
...
def _check_supabase_tables() -> list:
    ...
    for table in ("aisha_memory", "aisha_conversations", "aisha_mood_tracker"):
        try:
            r = requests.get(
                f"{base}/rest/v1/{table}?select=id&limit=1",  # Optimization: Added &limit=1 to count="exact" query to prevent loading all rows into memory
                headers={**headers, "Prefer": "count=exact"},
                timeout=10
            )
...
def _get_reliability_stats() -> dict:
    ...
    try:
        r1 = requests.get(
            f"{base}/rest/v1/aisha_system_log?level=eq.ERROR&created_at=gte.{since_1h}&select=id&limit=1",  # Optimization: Added &limit=1 to count="exact" query to prevent loading all rows into memory
            headers={**headers, "Prefer": "count=exact"},
            timeout=5,
        )
        if r1.ok:
    ...
    try:
        r2 = requests.get(
            f"{base}/rest/v1/aisha_system_log?level=eq.ERROR&created_at=gte.{since_24h}&select=id&limit=1",  # Optimization: Added &limit=1 to count="exact" query to prevent loading all rows into memory
            headers={**headers, "Prefer": "count=exact"},
            timeout=5,
        )
        if r2.ok:
    ...
    try:
        r3 = requests.get(
            f"{base}/rest/v1/content_jobs?status=eq.queued&select=id&limit=1",  # Optimization: Added &limit=1 to count="exact" query to prevent loading all rows into memory
            headers={**headers, "Prefer": "count=exact"},
            timeout=5,
        )
        if r3.ok:
