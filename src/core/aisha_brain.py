...
class AishaBrain:
    ...
    def get_semantic_intent(self, message_text: str) -> dict:
        """
        SHADOW MODE ROUTER: LLM Semantic Intent Classifier.
        Uses a fast model (nvidia/groq) to determine the true intent of the message.
        """
        import concurrent.futures
        import json
        import re

        prompt = f"""Analyze the following message and classify the user's intent.
Message: "{message_text}"

Return strictly valid JSON matching this exact schema:
{{
  "intent": "create_video" | "syscheck" | "chat" | "self_improve" | "api_search" | "unknown",
  "confidence": <float between 0.0 and 1.0>,
  "extracted_topic": "<string if video requested, else null>",
  "channel": "<guessed channel based on topic, else null>"
}}
"""
        def _run_classifier():
            return self.ai.generate(
                system_prompt="You are an intent classifier. Output ONLY raw valid JSON.",
                user_message=prompt,
                nvidia_task_type="fast"  # Use fast Llama-3 8B pool
            ).text

        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            try:
                future = executor.submit(_run_classifier)
                output = future.result(timeout=5)  # 5-second hard timeout
                cleaned = re.sub(r'```json\n?|```', '', output).strip()
                return json.loads(cleaned)
            except Exception as e:
                import logging
                log = logging.getLogger("Aisha.Brain")
                log.warning(f"Semantic Intent Classifier failed/timed out: {e}")
                return None

    def _detect_and_route_intent(self, user_message: str, is_owner: bool = True) -> str | None:
        """
        Check message against intent patterns.
        SHADOW MODE FALLBACK: Tries LLM classifier first. Falls back to Regex.
        """
        # 1. New LLM Intent Classifier (Shadow Mode)
        try:
            semantic_data = self.get_semantic_intent(user_message)
            if semantic_data and semantic_data.get("confidence", 0.0) > 0.85:
                intent = semantic_data.get("intent")
                if intent and intent not in ("chat", "unknown"):
                    import logging
                    logging.getLogger("Aisha.Brain").info(f"LLM routed intent: {intent}")
                    
                    # We map 'create_video' back to the old string for compatibility
                    mapped_intent = "content_creation" if intent == "create_video" else intent
                    
                    res = self._fire_intent(mapped_intent, user_message, is_owner=is_owner)
                    if res:
                        return res
        except Exception:
            pass  # Silently fall down to the Regex router
            
        # 2. Legacy Regex Router
        ...
        if intent == "content_creation":
            def _create():
                try:
                    ...
                    # Try to parse the channel if the LLM sent it as JSON, else use generic logic
                    channel = "Story With Aisha"
                    msg_lower = user_message.lower()
                    if "riya" in msg_lower or "dark" in msg_lower:
                        channel = "Riya's Dark Whisper"
                    elif "aisha & him" in msg_lower or "short" in msg_lower:
                        channel = "Aisha & Him"
                    ...
                    crew.kickoff({
                        ...
                        "format": "Short/Reel" if "short" in msg_lower or "reel" in msg_lower else "Long Form"
                    })
                except Exception as e:
                    import logging
                    logging.getLogger("Aisha.Brain").error(f"Background content creation failed: {e}")

            import threading
            threading.Thread(target=_create, daemon=True).start()
            return "Got it Ajju! Spinning up the Writers' Room... I'll ping you when the script is ready! 💜🎬" 
    ...
    def think(self, user_message: str, platform: str = "telegram",
        ...
        try:
            ...
            if intent == "create_video" or any(t in user_message.lower() for t in video_triggers):
                from src.core.queue_manager import queue_manager
                print(f"[Aisha] User requested video production. Enqueueing to Background Worker...")
                
                try:
                    # Enqueue safely without blocking the Telegram response
                    job_id = queue_manager.enqueue_job(
                        intent="create_video",
                        payload={"topic": user_message},
                        chat_id=caller_id
                    )
                except Exception as e:
                    print(f"[Brain] Failed to enqueue video job: {e}")
                    job_id = None
                
                if job_id:
                    response_text += "\n\nSure thing, Ajju! 💜 I've just queued the production crew on the studio floor. I'll notify you via Telegram the moment the first draft is ready for you! 🎬💸"
                else:
                    response_text += "\n\nArre Ajju, I tried to queue the production crew but there was a database glitch. Let me try again in a minute. 😅"
