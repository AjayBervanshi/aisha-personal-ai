...
def cmd_earnings(message):
    ...
    def _build_report():
        try:
            ...
            # All-time uploads
            # ⚡ Bolt: Optimized to use count="exact" and limit(1) to avoid downloading potentially thousands of rows (O(N) network/memory -> O(1))
            ...
            total_videos = all_uploaded_resp.count if hasattr(all_uploaded_resp, 'count') and all_uploaded_resp.count is not None else 0
